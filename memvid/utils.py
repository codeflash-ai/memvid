...
def _get_qr_capacity(version: int, error_correction: str) -> int:
    ...
    """
    # Rough capacity estimates (bytes) for different error correction levels
    capacities = {
    ...
    return capacities.get(error_correction, capacities['M'])[version - 1]
...
def qr_to_frame(qr_image: Image.Image, frame_size: Tuple[int, int]) -> np.ndarray:
    ...
    # Convert to numpy array and ensure proper dtype (avoid extra copy if already uint8)
    img_array = np.asarray(qr_image)
    ...
    # Compute min, max and unique value count efficiently in a single pass
    img_min = img_array.min()
    img_max = img_array.max()
    logger.debug(f"[qr_to_frame] Pixel value range: [{img_min}, {img_max}]")

    # For small integer-valued images, use bincount for contrast check instead of np.unique
    if img_array.dtype == np.uint8:
        # Fastest for uint8 images
        flat = img_array.ravel()
        nonzero_counts = np.count_nonzero(np.bincount(flat))
        unique_value_count = nonzero_counts
    else:
        unique_value_count = len(np.unique(img_array))
    logger.debug(f"[qr_to_frame] Unique pixel values count: {unique_value_count}")
    if unique_value_count < 2:
        logger.warning(f"[qr_to_frame] WARNING: Image has insufficient contrast! Only {unique_value_count} unique values")
